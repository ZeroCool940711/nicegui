...
def test_set_source_in_tab(screen: Screen):
    ...
    screen.wait(0.5)
    assert (
        screen.find_by_tag("img").get_attribute("src") == "https://nicegui.io/logo.png"
    )
    screen.click("B")
    screen.wait(0.5)
    screen.click("A")
...
def test_with_cross(screen: Screen, cross: bool, number_of_lines: int):
    ...
    ii.content = (
        '<circle cx="100" cy="100" r="15" fill="none" stroke="red" stroke-width="4" />'
    )
...
def test_mousemove_event(screen: Screen, cross: bool):
    ...
    ActionChains(screen.selenium).move_to_element_with_offset(element, 0, 0).pause(
        0.5
    ).move_by_offset(10, 10).pause(0.5).perform()
...
def test_loaded_event(screen: Screen):
    ...
    ui.button(
        "Change Source",
        on_click=lambda: ii.set_source("https://picsum.photos/id/30/640/360"),
    )
