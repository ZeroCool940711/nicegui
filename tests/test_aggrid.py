...
def test_click_cell(screen: Screen):
    ...
    grid.on(
        "cellClicked", lambda e: ui.label(f'{e.args["data"]["name"]} has been clicked!')
    )
...
def test_run_grid_method_with_argument(screen: Screen):
    ...
    ui.button(
        "Filter", on_click=lambda: grid.run_grid_method("setFilterModel", filter_model)
    )
...
def test_run_column_method_with_argument(screen: Screen):
    ...
    ui.button(
        "Show Age",
        on_click=lambda: grid.run_column_method("setColumnVisible", "age", True),
    )
...
def test_get_selected_rows(screen: Screen):
    ...
    screen.click("Alice")
    screen.find("Bob")
    ActionChains(screen.selenium).key_down(Keys.SHIFT).click(screen.find("Bob")).key_up(
        Keys.SHIFT
    ).perform()
...
def test_create_dynamically(screen: Screen):
    ui.button(
        "Create",
        on_click=lambda: ui.aggrid(
            {"columnDefs": [{"field": "name"}], "rowData": [{"name": "Alice"}]}
        ),
    )
...
def test_api_method_after_creation(screen: Screen):
    ...
    ui.button(
        "Create", on_click=lambda: ui.aggrid(options).run_grid_method("selectAll")
    )
...
def test_run_row_method(screen: Screen):
    ...
    ui.button(
        "Update",
        on_click=lambda: grid.run_row_method("Alice", "setDataValue", "age", 42),
    )
