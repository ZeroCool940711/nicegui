...
def test_multi_select(screen: Screen):
    s = ui.select(["Alice", "Bob", "Carol"], value="Alice", multiple=True).props(
        "use-chips"
    )
...
def test_add_new_values(
    screen: Screen, option_dict: bool, multiple: bool, new_value_mode: Optional[str]
):
    options = {"a": "A", "b": "B", "c": "C"} if option_dict else ["a", "b", "c"]
    if option_dict and new_value_mode == "add":
        with pytest.raises(
            ValueError, match='new_value_mode "add" is not supported for dict options'
        ):
    ...
    if new_value_mode:
        ...
        if new_value_mode == "add":
            screen.should_contain(
                "value = ['a', 'd', 'd']" if multiple else "value = d"
            )
            screen.should_contain(
                "options = {'a': 'A', 'b': 'B', 'c': 'C', 'd': 'd', 'd': 'd'}"
                if option_dict
                else "options = ['a', 'b', 'c', 'd', 'd']"
            )
...
def test_select_validation(screen: Screen):
    ui.select(
        ["A", "BC", "DEF"], value="A", validation={"Too long": lambda v: len(v) < 3}
    )
