...
def test_run_javascript_on_button_press(screen: Screen):
    ui.button(
        "change title",
        on_click=lambda: ui.run_javascript('document.title = "A New Title"'),
    )
...
def test_run_javascript_on_value_change(screen: Screen):
    ...
    async def page(client: Client):
        ui.radio(
            ["A", "B"],
            on_change=lambda e: ui.run_javascript(f'document.title = "Page {e.value}"'),
        )
...
def test_async_javascript(screen: Screen):
    async def run():
        result = await ui.run_javascript(
            "await new Promise(r => setTimeout(r, 100)); return 42"
        )
        ui.label(result)
...
def test_simultaneous_async_javascript(screen: Screen):
    ...
    async def runB():
        result = await ui.run_javascript(
            "await new Promise(r => setTimeout(r, 250)); return 2"
        )
        ui.label(f"B: {result}")
