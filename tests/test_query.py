...
def test_query_body(screen: Screen):
    ...
    ui.button(
        "Red background",
        on_click=lambda: ui.query("body").classes(replace="bg-red-100"),
    )
    ui.button(
        "Blue background",
        on_click=lambda: ui.query("body").classes(replace="bg-blue-100"),
    )
    ...
    assert (
        screen.find_by_tag("body").get_attribute("class")
        == "desktop no-touch body--light bg-orange-100"
    )
    ...
    assert (
        screen.find_by_tag("body").get_attribute("class")
        == "desktop no-touch body--light bg-red-100"
    )
    ...
    assert (
        screen.find_by_tag("body").get_attribute("class")
        == "desktop no-touch body--light bg-blue-100"
    )
...
def test_query_multiple_divs(screen: Screen):
    ...
    ui.label("B")
    ui.button(
        "Add border", on_click=lambda: ui.query("div").style("border: 1px solid black")
    )
