...
def test_classes(screen: Screen):
    ...
    def assert_classes(classes: str) -> None:
        assert screen.selenium.find_element(
            By.XPATH,
...
def test_style_parsing():
    ...
    assert ui.element._parse_style("color: red; background-color: blue") == {
        "color": "red",
        "background-color": "blue",
    }
    assert ui.element._parse_style("width:12em;height:34.5em") == {
        "width": "12em",
        "height": "34.5em",
    }
    assert ui.element._parse_style("transform: translate(120.0px, 50%)") == {
        "transform": "translate(120.0px, 50%)"
    }
    assert ui.element._parse_style("box-shadow: 0 0 0.5em #1976d2") == {
        "box-shadow": "0 0 0.5em #1976d2"
    }
...
def test_props_parsing():
    ...
    assert ui.element._parse_props('one two=1 three="abc def"') == {
        "one": True,
        "two": "1",
        "three": "abc def",
    }
    assert ui.element._parse_props("loading percentage=12.5") == {
        "loading": True,
        "percentage": "12.5",
    }
    assert ui.element._parse_props("size=50%") == {"size": "50%"}
    assert ui.element._parse_props("href=http://192.168.42.100/") == {
        "href": "http://192.168.42.100/"
    }
    assert ui.element._parse_props('hint="Your \\"given\\" name"') == {
        "hint": 'Your "given" name'
    }
    assert ui.element._parse_props('input-style="{ color: #ff0000 }"') == {
    ...
    assert ui.element._parse_props('''three='abc def' four="hhh jjj"''') == {
        "three": "abc def",
        "four": "hhh jjj",
    }
    assert ui.element._parse_props('''foo="quote'quote"''') == {"foo": "quote'quote"}
    assert ui.element._parse_props("""foo='quote"quote'""") == {"foo": 'quote"quote'}
    assert ui.element._parse_props("""foo="single '" bar='double "'""") == {
        "foo": "single '",
        "bar": 'double "',
    }
    assert ui.element._parse_props("""foo="single '" bar='double \\"'""") == {
    ...
    assert ui.element._parse_props("""input-style='{ myquote: "quote" }'""") == {
        "input-style": '{ myquote: "quote" }'
    }
...
def test_style(screen: Screen):
    ...
    def assert_style(style: str) -> None:
        assert screen.selenium.find_element(
            By.XPATH, f'//*[normalize-space(@style)="{style}" and text()="Some label"]'
        )
...
def test_props(screen: Screen):
    ...
    def assert_props(*props: str) -> None:
        ...
        assert screen.selenium.find_element(
            By.XPATH, f'//label[{" and ".join(class_conditions)}]'
        )
...
def test_move(screen: Screen):
    ...
    screen.open("/")
    assert (
        screen.find("A").location["y"]
        < screen.find("X").location["y"]
        < screen.find("B").location["y"]
    )
...
def test_default_props():
    ...
    assert (
        input_a._props.get("rounded") is None
    ), "default props of ui.button do not affect ui.input"
    ...
    assert (
        button_e._props.get("flat") is None
    ), "default props of MyButton do not affect ui.button"
...
def test_default_classes():
    ...
    assert (
        "bg-white" not in input_a._classes
    ), "default classes of ui.button do not affect ui.input"
    ...
    assert (
        "w-full" not in button_e._classes
    ), "default classes of MyButton do not affect ui.button"
...
def test_default_style():
    ...
    assert (
        input_a._style.get("font-size") is None
    ), "default style of ui.button does not affect ui.input"
    ...
    assert (
        button_e._style.get("font-family") is None
    ), "default style of MyButton does not affect ui.button"
