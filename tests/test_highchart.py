...
def test_change_chart_series(screen: Screen):
    ...
    def get_series_0():
        return screen.selenium.find_elements(
            By.CSS_SELECTOR, ".highcharts-series-0 .highcharts-point"
        )
...
def test_stock_chart(screen: Screen):
    ...
    screen.open("/")
    assert screen.selenium.find_elements(
        By.CSS_SELECTOR, ".highcharts-range-selector-buttons"
    )
...
def test_updating_stock_chart(screen: Screen):
    ...
    chart = ui.highchart(
        {"legend": {"enabled": True}, "series": []}, type="stockChart", extras=["stock"]
    )
    ui.button(
        "update",
        on_click=lambda: (
