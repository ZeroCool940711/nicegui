...
def test_warning_about_to_late_responses(screen: Screen):
    ...
    screen.assert_py_logger(
        "ERROR",
        re.compile("it was returned after the HTML had been delivered to the client"),
    )
...
def test_reconnecting_without_page_reload(screen: Screen):
    ...
    def page():
        ...
        ui.button(
            "drop connection",
            on_click=lambda: ui.run_javascript("socket.io.engine.close()"),
        )
    ...
    assert (
        element.get_attribute("value") == "hello"
    ), "input should be preserved after reconnect (i.e. no page reload)"
