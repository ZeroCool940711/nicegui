...
def test_toggle_button(screen: Screen):
    ui.input(
        "Your password", value="123456", password=True, password_toggle_button=True
    )
...
def test_input_with_multi_word_error_message(screen: Screen):
    ...
    ui.button(
        "set error",
        on_click=lambda: input_.props(
            'error error-message="Some multi word error message"'
        ),
    )
