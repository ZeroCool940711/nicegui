        ...
        for _ in range(3):
            try:
                pos = screen.selenium.execute_script(
                    f'return scene_c{scene.id}.getObjectByName("sphere").position.z'
                )
                if pos is not None:
                    return pos
...
def test_no_object_duplication_on_index_client(screen: Screen):
    ...
    screen.wait(0.2)
    assert (
        screen.selenium.execute_script(f"return scene_c{scene.id}.children.length") == 5
    )
...
def test_replace_scene(screen: Screen):
    ...
    screen.wait(0.5)
    assert (
        screen.selenium.execute_script(f"return scene_c{scene.id}.children[4].name")
        == "sphere"
    )
