...
class page:
    ...
    def __call__(self, func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        """
        core.app.remove_route(
            self.path
        )  # NOTE make sure only the latest route definition is used
        parameters_of_decorated_func = list(inspect.signature(func).parameters.keys())

        def check_for_late_return_value(task: asyncio.Task) -> None:
            if task.result() is not None:
                log.error(
                    f"ignoring {task.result()}; it was returned after the HTML had been delivered to the client"
                )
        ...
        async def decorated(*dec_args, **dec_kwargs) -> Response:
            ...
            dec_kwargs = {
                k: v for k, v in dec_kwargs.items() if k in parameters_of_decorated_func
            }
            ...
            if helpers.is_coroutine_function(func):
                ...
                while task and not client.is_waiting_for_connection and not task.done():
                    if time.time() > deadline:
                        raise TimeoutError(
                            f"Response not ready after {self.response_timeout} seconds"
                        )
            ...
            if isinstance(
                result, Response
            ):  # NOTE if setup returns a response, we don't need to render the page
                return result
            binding._refresh_step()  # pylint: disable=protected-access
            return client.build_response(request)

        parameters = [
            p for p in inspect.signature(func).parameters.values() if p.name != "client"
        ]
        ...
        if "include_in_schema" not in self.kwargs:
            self.kwargs["include_in_schema"] = (
                core.app.config.endpoint_documentation in {"page", "all"}
            )
