...
class KeyboardKey:
    ...
    def number(self) -> Optional[int]:
        ...
        return (
            int(self.code.removeprefix("Digit"))
            if self.code.startswith("Digit")
            else None
        )
...
def handle_event(
    handler: Optional[Callable[..., Any]], arguments: EventArguments
) -> None:
    ...
    try:
        ...
        if isinstance(arguments, UiEventArguments):
            if arguments.sender.is_ignoring_events:
                return
            parent_slot = (
                arguments.sender.parent_slot
                or arguments.sender.client.layout.default_slot
            )
        else:
