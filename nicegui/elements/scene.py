...
class Scene(
    ...
    def run_method(
        ...
        if not self.is_initialized:
            return NullResponse()
        return super().run_method(
            name, *args, timeout=timeout, check_interval=check_interval
        )
    ...
    def _handle_drag(self, e: GenericEventArguments) -> None:
        ...
        if arguments.type == "dragend":
            self.objects[arguments.object_id].move(
                arguments.x, arguments.y, arguments.z
            )
        handle_event(
            self._drag_start_handler
            if arguments.type == "dragstart"
            else self._drag_end_handler,
            arguments,
        )
    ...
    def move_camera(
        ...
        self.camera.look_at_x = (
            self.camera.look_at_x if look_at_x is None else look_at_x
        )
        self.camera.look_at_y = (
            self.camera.look_at_y if look_at_y is None else look_at_y
        )
        self.camera.look_at_z = (
            self.camera.look_at_z if look_at_z is None else look_at_z
        )
    ...
    def delete_objects(
        self, predicate: Callable[[Object3D], bool] = lambda _: True
    ) -> None:
