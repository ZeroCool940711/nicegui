...
class Select(
    ValidationElement, ChoiceElement, DisableableElement, component="select.js"
):
    def __init__(
        self,
        ...
        *,
        ...
        if multiple:
            if value is None:
                ...
                value = [value]
        super().__init__(
            options=options, value=value, on_change=on_change, validation=validation
        )
        ...
        if new_value_mode is not None:
            if isinstance(options, dict) and new_value_mode == "add":
                raise ValueError(
                    'new_value_mode "add" is not supported for dict options'
                )
    ...
    def _event_args_to_value(self, e: GenericEventArguments) -> Any:
        ...
        if self.multiple:
            if e.args is None:
                ...
                args = [
                    self._values[arg["value"]] if isinstance(arg, dict) else arg
                    for arg in e.args
                ]
                for arg in e.args:
