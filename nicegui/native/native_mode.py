...
def _start_window_method_executor(
    ...
    def execute(
        method: Callable, args: Tuple[Any, ...], kwargs: Dict[str, Any]
    ) -> None:
        try:
            ...
            try:
                ...
                if method_name == "signal_server_shutdown":
                    if pending_executions:
                        log.warning(
                            "shutdown is possibly blocked by opened dialogs like a file picker"
                        )
                    ...
                    if callable(method):
                        pending_executions.append(
                            Thread(target=execute, args=(method, args, kwargs))
                        )
