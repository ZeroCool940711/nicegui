...
class App(FastAPI):
    ...
    def on_startup(self, handler: Union[Callable, Awaitable]) -> None:
        ...
        if self.is_started:
            raise RuntimeError(
                "Unable to register another startup handler. NiceGUI has already been started."
            )
    ...
    def shutdown(self) -> None:
        ...
        if self.config.reload:
            raise RuntimeError(
                "calling shutdown() is not supported when auto-reload is enabled"
            )
    ...
    def add_static_files(
        ...
        *,
        ...
        if url_path == "/":
            raise ValueError(
                """Path cannot be "/", because it would hide NiceGUI's internal "/_nicegui" route."""
            )
        self.mount(
            url_path,
            StaticFiles(directory=str(local_directory), follow_symlink=follow_symlink),
        )
    ...
    def add_static_file(
        ...
        *,
        ...
        path = (
            f"/_nicegui/auto/static/{helpers.hash_file_path(file)}/{file.name}"
            if url_path is None
            else url_path
        )
    ...
    def add_media_files(self, url_path: str, local_directory: Union[str, Path]) -> None:
        ...
        def read_item(
            request: Request, filename: str, nicegui_cunk_size: int = 8192
        ) -> Response:
    ...
    def add_media_file(
        ...
        *,
        ...
        path = (
            f"/_nicegui/auto/media/{helpers.hash_file_path(file)}/{file.name}"
            if url_path is None
            else url_path
        )
