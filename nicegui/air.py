...
class Air:
    def __init__(self, token: str) -> None:
        ...
        async def _handle_http(data: Dict[str, Any]) -> Dict[str, Any]:
            ...
            headers.update(
                {"Accept-Encoding": "identity", "X-Forwarded-Prefix": data["prefix"]}
            )
            ...
            if match:
                ...
                content = content.replace(
                    match.group(0), f"const query = {new_js_object}".encode()
                )
        ...
        async def _handle_range_request(data: Dict[str, Any]) -> Dict[str, Any]:
            headers: Dict[str, Any] = data["headers"]
            url = (
                list(u for u in core.app.urls if self.remote_url != u)[0] + data["path"]
            )
            ...
            self.streams[stream_id] = Stream(
                data=response.aiter_bytes(), response=response
            )
            return {
                "status_code": response.status_code,
                "headers": response.headers.multi_items(),
        ...
        def _handle_event(data: Dict[str, Any]) -> None:
            ...
            if client_id not in Client.instances:
                return
            client = Client.instances[client_id]
            if data["msg"]["args"] and data["msg"]["args"][0].startswith(
                '{"socket_id":'
            ):
                args = json.loads(data["msg"]["args"][0])
                args[
                    "socket_id"
                ] = client_id  # HACK: translate socket_id of ui.scene's init event
        ...
        async def _handle_out_of_time() -> None:
            print(
                "Sorry, you have reached the time limit of this NiceGUI On Air preview.",
                flush=True,
            )
            await self.connect()
    ...
    async def emit(self, message_type: str, data: Dict[str, Any], room: str) -> None:
        ...
        if self.relay.connected:
            await self.relay.emit(
                "forward", {"event": message_type, "data": data, "room": room}
            )
