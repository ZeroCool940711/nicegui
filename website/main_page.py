...
def create() -> None:
    ...
    with ui.row().classes(
        ...
        svg.face(half=True).classes(
            "stroke-black dark:stroke-white w-[200px] md:w-[230px] lg:w-[300px]"
        )
    ...
    with ui.column().classes("w-full text-lg p-8 lg:p-16 max-w-[1600px] mx-auto"):
        ...
        with ui.row().classes(
            "w-full text-lg leading-tight grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8"
        ):
            ...
            with ui.column().classes("w-full max-w-md gap-2"):
                ...
                with documentation.browser_window(classes="w-full h-52"):
                    ui.label("Hello NiceGUI!")
        with ui.expansion("...or use Docker to run your main.py").classes(
            "w-full gap-2 bold-links arrow-links"
        ):
    ...
    with ui.column().classes(
        "w-full p-8 lg:p-16 bold-links arrow-links max-w-[1600px] mx-auto"
    ):
        link_target("features", "-50px")
        section_heading("Features", "Code *nicely*")
        with ui.row().classes(
            "w-full text-lg leading-tight grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8"
        ):
            features(
                "swap_horiz",
                "Interaction",
    ...
    with ui.column().classes("dark-box p-8 lg:p-16 my-16"):
        with ui.column().classes("mx-auto items-center gap-y-8 gap-x-32 lg:flex-row"):
            with ui.column().classes("gap-1 max-lg:items-center max-lg:text-center"):
                ui.markdown("Browse through plenty of live demos.").classes(
            ...
            ui.link("Documentation", "/documentation").style(
                "color: black !important"
            ).classes(
                "rounded-full mx-auto px-12 py-2 bg-white font-medium text-lg md:text-xl"
            )
    ...
    with ui.column().classes("w-full p-8 lg:p-16 max-w-[1600px] mx-auto"):
        ...
        with ui.row().classes(
            "w-full text-lg leading-tight grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4"
        ):
    ...
    with ui.row().classes("dark-box min-h-screen mt-16"):
        ...
        with ui.column().classes(
            ...
            with ui.column().classes("gap-8"):
                heading("Why?")
                with ui.column().classes(
                    "gap-2 text-xl text-white bold-links arrow-links"
                ):
                    ui.markdown(
                        """
                        We at
            ...
            svg.face().classes(
                "stroke-white shrink-0 w-[200px] md:w-[300px] lg:w-[450px]"
            )
