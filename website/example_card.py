...
def create() -> None:
    ...
    with ui.row().style("filter: drop-shadow(0 1px 2px rgba(0, 0, 0, 0.1))"):
        with ui.card().style(
            r"clip-path: polygon(0 0, 100% 0, 100% 90%, 0 100%)"
        ).classes("pb-16 no-shadow"), ui.row().classes("no-wrap"):
            with ui.column().classes("items-center"):
                svg.face().classes(
                    "w-16 mx-6 stroke-black dark:stroke-gray-100 stroke-2"
                ).on("click", lambda _: output.set_text("That's my face!"), [])
                ui.button(
                    "Click me!", on_click=lambda: output.set_text("Clicked")
                ).classes("w-full")
                ui.input(
                    "Text", value="abc", on_change=lambda e: output.set_text(e.value)
                )
                ui.checkbox(
                    "Check",
                    on_change=lambda e: output.set_text(
                ...
                ui.switch(
                    "Switch",
                    on_change=lambda e: output.set_text(
                        "Switched on" if e.value else "Switched off"
                    ),
                )
...
def create_narrow() -> None:
    ...
    with ui.row().style("filter: drop-shadow(0 1px 2px rgba(0, 0, 0, 0.1))"):
        with ui.card().style(
            ...
            with ui.column().classes("items-center"):
                svg.face().classes("w-16 mx-6 stroke-black stroke-2").on(
                    "click", lambda _: output.set_text("That's my face!")
                )
                output = ui.label("Try it out!").classes(
                    "w-44 my-6 h-8 text-xl text-gray-800 dark:text-gray-200 overflow-hidden text-ellipsis text-center"
                )
