...
def methods_demo() -> None:
    ...
    ui.button(
        "Expand", on_click=lambda: editor.run_editor_method(":expand", "path => true")
    )
    ui.button(
        "Collapse",
        on_click=lambda: editor.run_editor_method(":expand", "path => false"),
    )
    ui.button(
        "Readonly",
        on_click=lambda: editor.run_editor_method("updateProps", {"readOnly": True}),
    )
