...
def page_visits():
    ...
    counter[app.storage.browser["id"]] += 1
    ui.label(
        f"{len(counter)} unique views ({sum(counter.values())} overall) since {start}"
    )
...
def ui_state():
    ...
    # END OF DEMO
    ui.textarea("This note is kept between visits").classes("w-full").bind_value(
        app.storage.user, "note"
    )
