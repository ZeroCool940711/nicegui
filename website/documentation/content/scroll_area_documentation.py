...
def scroll_events():
    ...
    with ui.card().classes("w-32 h-32"):
        with ui.scroll_area(
            on_scroll=lambda e: position.set_value(e.vertical_percentage)
        ):
            ui.label("I scroll. " * 20)
...
def scroll_position():
    ui.number(
        ...
        max=1,
        step=0.1,
        on_change=lambda e: area1.scroll_to(percent=e.value),
    ).classes("w-32")

    with ui.row():
        with ui.card().classes("w-32 h-48"):
            with ui.scroll_area(
                on_scroll=lambda e: area2.scroll_to(percent=e.vertical_percentage)
            ) as area1:
                ui.label("I scroll. " * 20)
