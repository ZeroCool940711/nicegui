...
def search_as_you_type():
    ...
    ui.select(
        options=continents, with_input=True, on_change=lambda e: ui.notify(e.value)
    ).classes("w-40")
...
def multi_select():
    ...
    ui.select(names, multiple=True, value=names[:2], label="comma-separated").classes(
        "w-64"
    )
    ui.select(names, multiple=True, value=names[:2], label="with chips").classes(
        "w-64"
    ).props("use-chips")
