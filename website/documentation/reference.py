...
def generate_class_doc(class_obj: type, part_title: str) -> None:
    ...
    methods = {
        name: attribute for name, attribute in attributes.items() if callable(attribute)
    }

    if properties:
        subheading(
            "Properties",
            anchor_name=create_anchor_name(
                part_title.replace("Reference", "Properties")
            ),
        )
        with ui.column().classes("gap-2"):
            for name, property_ in sorted(properties.items()):
                ui.markdown(
                    f"**`{name}`**`{_generate_property_signature_description(property_)}`"
                )
    ...
    if methods:
        subheading(
            "Methods",
            anchor_name=create_anchor_name(part_title.replace("Reference", "Methods")),
        )
        with ui.column().classes("gap-2"):
            for name, method in sorted(methods.items()):
                ...
                ui.markdown(
                    f"{decorator}**`{name}`**`{_generate_method_signature_description(method)}`"
                )
                if method.__doc__:
    ...
    if ancestors:
        subheading(
            "Inheritance",
            anchor_name=create_anchor_name(
                part_title.replace("Reference", "Inheritance")
            ),
        )
...
def _generate_method_signature_description(method: Callable) -> str:
    ...
    for param in inspect.signature(method).parameters.values():
        ...
        if param.default != inspect.Parameter.empty:
            param_string += (
                " = [...]" if callable(param.default) else f" = {repr(param.default)}"
            )
