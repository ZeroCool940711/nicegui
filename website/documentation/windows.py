...
def _window(
    ...
    *,
    ...
    with ui.card().classes(
        f"no-wrap bg-[{color[0]}] dark:bg-[{color[1]}] rounded-xl p-0 gap-0 {classes}"
    ).style("box-shadow: 0 1px 2px rgba(0, 0, 0, 0.1)"):
        with ui.row().classes(
            f"w-full h-8 p-2 bg-[{bar_color[0]}] dark:bg-[{bar_color[1]}]"
        ):
            _dots()
            if title:
                ui.label(title).classes(
                    "text-sm text-gray-600 dark:text-gray-400 absolute left-1/2 top-[6px]"
                ).style("transform: translateX(-50%)")
            if tab:
                with ui.row().classes("gap-0"):
                    with ui.label().classes(
                        f"w-2 h-[24px] bg-[{color[0]}] dark:bg-[{color[1]}]"
                    ):
                        ui.label().classes(
                            f"w-full h-full bg-[{bar_color[0]}] dark:bg-[{bar_color[1]}] rounded-br-[6px]"
                        )
                    with ui.row().classes(
                        f"text-sm text-gray-600 dark:text-gray-400 px-6 py-1 h-[24px] rounded-t-[6px] bg-[{color[0]}] dark:bg-[{color[1]}] items-center gap-2"
                    ):
                        if callable(tab):
                            tab()
                        else:
...
def python_window(title: Optional[str] = None, *, classes: str = "") -> ui.column:
    ...
    """
    return _window("python", title=title or "main.py", classes=classes).classes(
        "px-4 py-2 python-window"
    )
...
def bash_window(*, classes: str = "") -> ui.column:
    ...
    """
    return _window("bash", title="bash", classes=classes).classes(
        "px-4 py-2 bash-window"
    )


def browser_window(
    title: Optional[Union[str, Callable]] = None, *, classes: str = ""
) -> ui.column:
    """
    ...
    """
    return _window("browser", tab=title or "NiceGUI", classes=classes).classes(
        "p-4 browser-window"
    )
