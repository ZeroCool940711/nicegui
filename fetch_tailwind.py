...
class Property:
    ...
    def pascal_title(self) -> str:
        ...
        """
        return "".join(
            word.capitalize() for word in re.sub(r"[-/ &]", " ", self.title).split()
        )
    ...
    def snake_title(self) -> str:
        ...
        """
        return "_".join(
            word.lower() for word in re.sub(r"[-/ &]", " ", self.title).split()
        )
...
def collect_properties() -> List[Property]:
    ...
    for li in soup.select('li[class="mt-12 lg:mt-8"]'):
        ...
        for a in links:
            ...
            properties.append(
                Property(title, description, [p.text.split(" ")[0] for p in members])
            )
    ...
    with (Path(__file__).parent / "nicegui" / "tailwind.py").open("w") as f:
        ...
        for property_ in sorted(properties, key=lambda p: p.title):
            if not property_.members:
                continue
            f.write(
                f"    from .tailwind_types.{property_.snake_title} import {property_.pascal_title}\n"
            )
        f.write("\n")
        f.write("\n")
        ...
        for property_ in properties:
            ...
            if property_.members:
                f.write(
                    f"    def {property_.snake_title}(self, value: {property_.pascal_title}) -> Tailwind:\n"
                )
