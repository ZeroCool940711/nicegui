...
parser = argparse.ArgumentParser(
    description="Fetch the content of a milestone from a GitHub repo."
)
...
milestones = requests.get(
    f"{BASE_URL}/milestones?state=all&per_page=100", timeout=5
).json()
matching_milestones = [
...
def link(number: int) -> str:
    ...
    return escape_mask.format(
        "", f"https://github.com/zauberzeug/nicegui/issues/{number}", f"#{number}"
    )


issues = requests.get(
    f"{BASE_URL}/issues?milestone={milestone_number}&state=all", timeout=5
).json()
...
for issue in issues:
    ...
    number_patterns = [
        r"#(\d+)",
        r"https://github.com/zauberzeug/nicegui/(?:issues|discussions|pulls)/(\d+)",
    ]
    numbers = [issue["number"]] + [
        int(match) for pattern in number_patterns for match in re.findall(pattern, body)
    ]
