...
css += (
    "\n"
    + requests.get(
        f'https://fonts.googleapis.com/css?family={"|".join(css_families)}',
...
css += (
    "\n"
    + requests.get(
        f'https://fonts.googleapis.com/css?family={"|".join(css2_families)}',
        headers={"User-Agent": AGENT},
        timeout=5,
    ).content.decode()
)
for font_url in re.findall(r"url\((.*?)\)", css):
    ...
    filepath = filepath.with_stem(
        hashlib.sha256(filepath.stem.encode()).hexdigest()[:16]
    )
    if filepath.exists():
